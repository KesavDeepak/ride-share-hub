  ...
  useEffect(() => {
    ...
    class App {
      constructor(container, options = {}) {
        ...
        this.speedUp = 0;
        this.timeOffset = 0;
        ['tick','init','setSize','onMouseDown','onMouseUp','onTouchStart','onTouchEnd','onContextMenu','onWindowResize'].forEach(m => this[m] = this[m].bind(this));
      ...
      loadAssets() {
        return new Promise(resolve => {
          ...
          s.src = SMAAEffect.searchImageDataURL; a.src = SMAAEffect.areaImageDataURL;
        });
      }
      init() {
        this.initPasses();
      ...
      dispose() {
        ...
        if (this.container){
          ['mousedown','mouseup','mouseout','touchstart','touchend','touchcancel','contextmenu'].forEach(ev => this.container.removeEventListener(ev, this['on'+ev.charAt(0).toUpperCase()+ev.slice(1)]));
        }
      }
      setSize(w,h,u){ if (w<=0||h<=0){ this.hasValidSize=false; return; } this.composer.setSize(w,h,u); this.hasValidSize=true; }
      tick() {
    ...
    class CarLights {
      constructor(w,o,c,s,f){ this.webgl=w; this.options=o; this.colors=c; this.speed=s; this.fade=f; }
      init() {
        ...
        material.onBeforeCompile = sh => { sh.vertexShader = sh.vertexShader.replace('#include <getDistortion_vertex>', options.distortion.getDistortion); };
    ...
    class LightsSticks {
      constructor(w,o){ this.webgl=w; this.options=o; }
      init() {
        const options = this.options;
    ...
    class Road {
      constructor(w,o){ this.webgl=w; this.options=o; this.uTime={value:0}; }
